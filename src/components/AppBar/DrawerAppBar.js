...
function DrawerAppBar(props) {
  ...
  const [showSignupModal, setShowSignupModal] = React.useState(false); // State for Signup modal
  ...
  const handleLogout = () => {
    ...
    if (!token) setShowL(true);
  };

  const handleSignupModal = () => {
    setShowS(true); // Set showS to true when "Sign Up" is clicked
            ...
            {item.label !== 'Logout' ? (
              <Link
                ...
                style={{
                  ...
                  backgroundColor: 'red',
                  marginTop: '10px',
                  borderRadius: item.label === 'Sign Up' || item.label === 'Logout' ? '10px' : undefined,
                  background: item.label === 'Sign Up' || item.label === 'Logout' ? '#850202' : undefined,
                  padding: item.label === 'Sign Up' || item.label === 'Logout' ? '10px' : undefined,
                  color: item.label === 'Sign Up' || item.label === 'Logout' ? 'white' : '#850202',
                }}
                to={item.link}
              >
                <ListItem
                  ...
                  sx={{
                    textAlign: 'center',
                    borderBottom: selectedNavItem === item.label ? '2px solid red' : 'none',
                  }}
                >
                  <ListItemButton
                    sx={{
                      textAlign: 'center',
                      borderBottom: selectedNavItem === item.label ? '4px solid red' : 'none',
                    }}
                    onClick={item.label === 'Logout' ? openModal : (item.label === 'Sign Up' ? handleSignupModal : undefined)}
                  >
                    <ListItemText
                      primary={item.label}
            ...
            <Box sx={{ display: { xs: 'none', sm: 'flex' }, alignItems: 'center' }}>
              {navItems.map((item, index) => (
                <React.Fragment key={index}>
                  ...
                  {item.label === 'Sign Up' && showS && (
                    <SignupModal
                      showModal={showSignupModal}
                      handleClose={() => setShowSignupModal(false)}
                      // You may need to pass additional props to SignupModal
                    />
                  )}
                </React.Fragment>
              ))}
