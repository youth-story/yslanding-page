    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.group}>
                {daysRemaining > 0 ? (
                    <p className={styles.para}><span className={styles.daysCount}>{daysRemaining}</span> {daysRemaining === 1 ? ' day' : ' days'} <span style={{color: 'greenyellow'}}>untill launch</span></p>
                ) : (
                    <p className={styles.launched}>We are launching today!</p>
                )}
                <button className={styles.btn} onClick={scroller}>Know More</button>
            </div>
        </div>
    );
